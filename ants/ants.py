        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.container and self.ant.can_contain(insect):
                    self.ant.ant=insect
                elif insect.container==True and insect.can_contain(self.ant):
                    insect.ant=self.ant
                    self.ant=insect
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.place.bees:
            ...
            while(previous_place!=hive and previous_place!=None):
                if previous_place.bees:
                    return random_or_none(previous_place.bees)
                previous_place = previous_place.entrance
            return None
        # END Problem 3 and 4
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #注意，上面不能写成self.add_insect(insect),因为此时的self是Place的继承类Water的一个instance
        #它还没有add_insect()这个函数（是我们正在定义的），所以要调用Place的add_insect(self,insect)函数，
        #把self传进去当做参数就可以了。
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            ...
            for bee in self.place.bees:
                bee.armor = bee.armor-self.damage
                if bee.armor>0:
                    bees.append(bee)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self,hive):
        previous_place = self.place.entrance
        i=1
        while(previous_place!=hive and previous_place!=None and i<self.min_range):
            previous_place = previous_place.entrance
            i=i+1
        if i==self.min_range and previous_place:
        #现在previous_place就是可以开始攻击的位置（第五个位置）
            start_attack_place = previous_place
            if start_attack_place.bees:
                return random_or_none(start_attack_place.bees)
            else:
                previous_place = start_attack_place.entrance
                while(previous_place!=hive and previous_place!=None):
                    if previous_place.bees:
                        return random_or_none(previous_place.bees)
                    previous_place = previous_place.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        if self.place.bees:
            ...
            previous_place = self.place.entrance
            i=0
            while(previous_place!=hive and previous_place!=None and i<self.max_range):
                if previous_place.bees:
                    return random_or_none(previous_place.bees)
                previous_place = previous_place.entrance
                i=i+1
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        if bees:
            for bee in bees:
                bee.armor=bee.armor-self.damage
            self.place.bees=[bee for bee in bees if bee.armor>0]
        # END Problem 7
...
class ScubaThrower(ThrowerAnt):
    ...
    def reduce_armor(self):
        ThrowerAnt.reduce_armor(self,0)
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while(previous_place):
                if previous_place.ant!=None:
                    if previous_place.ant not in self.double_list:
                        previous_place.ant.damage = previous_place.ant.damage*2
                        self.double_list.append(previous_place.ant)
                        if previous_place.ant.container is True and previous_place.ant.ant!=None:
                            if previous_place.ant.ant not in self.double_list:
                                previous_place.ant.ant.damage = previous_place.ant.ant.damage*2
                                self.double_list.append(previous_place.ant.ant)
                    elif previous_place.ant.container is True and previous_place.ant.ant!=None:
                        if previous_place.ant.ant not in self.double_list:
                            previous_place.ant.ant.damage = previous_place.ant.ant.damage*2
                            self.double_list.append(previous_place.ant.ant)
                previous_place=previous_place.exit
            if self.place.ant.container is True and self.place.ant not in self.double_list:
                self.place.ant.damage = self.place.ant.damage*2
                self.double_list.append(self.place.ant)
        # END Problem 13
