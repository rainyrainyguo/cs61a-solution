...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        t=0
        while self.balance*(1+self.interest)**t<amount:
            t=t+1
        return t
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    error=withdraw(0,old_password) #calling withdraw here actually records an attempt for password.No need to return withdraw to record a wrong password
    if type(error)==str:
        return error
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    #assert lower_bound(y)*upper_bound(y)>0 这个不是很严谨，比如(5,3)
...
def check_par():
    ...
    r2 = interval(2,6) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    r2=f(upper_bound(x))
    t=-b/(2*a)
    if t>=lower_bound(x) and t<=upper_bound(x):
        return interval(min(r1,r2,f(t)),max(r1,r2,f(t)))
